...
在模型中点选一条用户绘制的 **竖直线**（立杆轴线），并在面板上输入净距 **B**，
据此生成一组门型架：
...
横担**横跨两根立柱、两端各超出立柱外缘 15 mm**，顶面（固定管子的面）落在
所选竖直线的顶端（即高度 H 处）。立柱与横担**背靠背**：
...
DEBUG_LOG = os.path.join(HERE, '门型架_debug_log.txt')
...
def extract_vertical_post(element_handle):
    """从所选元素提取并校验竖直线（立杆轴线），返回 ``门型架_几何.VerticalPost``。"""
...
def _build_portal_frame_cell(post, variant_key, rack_type, span_mm, heading_deg,
    ...
    # 两立柱：左立柱轴线即所选竖直线，右立柱轴线在 u = B + W 处。
...
def export_bom_json(output_path=None):
    ...
    if output_path is None:
        output_path = os.path.join(HERE, '门型架_bom.json')
...
class _PortalFrameSettingsDialog(QWidget):
    ...
    def __init__(self):
        ...
        hint = QLabel("在模型中点选一条竖直线：线长即门架高 H，左立柱轴线与该线"
                      "重合。再输入两立柱净距 B —— 横担长按 L = B + 2×立柱宽度 "
                      "+ 30 自动计算（横担横跨两立柱、两端各超立柱外缘 15；"
                      "角钢/槽钢立柱都与横担背靠背、非通长，顶端留 10 焊缝间隙）。"
                      "竖直线不能定出朝向，请用「朝向」"
...
def RegisterKeyins():
    ...
    if _COMMANDS_LOADED:
        return
    command_xml = os.path.join(HERE, '门型架（角钢和槽钢）.commands.xml')
