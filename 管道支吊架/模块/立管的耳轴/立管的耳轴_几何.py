...
# =============================================================================
# 【公共模块 · 请勿直接运行】
# 本文件仅作为纯数据 / 尺寸推导库供 ``F6_F7-[立管的耳轴].py`` 等插件 ``import`` 调用，
...
``F6_F7-[立管的耳轴].py`` 调用；只做**数据与尺寸**，不涉及任何 Bentley 几何调用。
...
* **补强板**：贴主管外壁、围绕耳轴根部的**弯曲圆形钢板**，外径 = **耳轴外径 + 2W**
  （W 查补强板表，按主管管径分档），厚度为变量（暂按 5 mm）；由面板勾选是否建，
  缺省不建（注 11）；越过主管中心线的部分切至与中心线平齐（注 7）；
