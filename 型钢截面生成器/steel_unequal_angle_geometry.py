"""True-arc geometry for unequal-leg rolled angle sections."""
...
from steel_unequal_angle_data import validate_section
...
INSERTION_MODES = (
    ...
    (INSERTION_GEOMETRIC_CENTER, "外接矩形中心"),
    (INSERTION_OUTER_CORNER, "两腿外侧交点"),
)


class UnequalAngleGeometry(object):
    """Closed unequal-angle boundary with one r1 and two r2 true arcs."""
...
def scale_section(section, factor):
    ...
    scaled = dict(section)
    for name in ("BL", "BS", "t", "r1", "r2", "X0", "Y0"):
...
def insertion_anchor(section, insertion_mode):
    ...
    if insertion_mode == INSERTION_CENTROID:
        return Point2d(float(section["X0"]), float(section["Y0"]))
    if insertion_mode == INSERTION_GEOMETRIC_CENTER:
        return Point2d(float(section["BS"]) / 2.0, float(section["BL"]) / 2.0)
    if insertion_mode == INSERTION_OUTER_CORNER:
        return Point2d(0.0, 0.0)
    raise ValueError("Unsupported insertion mode: {}".format(insertion_mode))


def build_unequal_angle_geometry(section, insertion_mode=INSERTION_CENTROID,
                                 insertion_point=Point2d(0.0, 0.0)):
    """Build an unequal angle with BL vertical and BS horizontal.

    The long leg grows in positive Y and the short leg grows in positive X.
    This follows the Table A.5 axes: X0 is horizontal and Y0 is vertical from
    the two outer leg faces to the tabulated centroid.
    """
    validate_section(section)
    ...
    if not geometry.is_closed_and_continuous():
        raise RuntimeError("Internal error: generated unequal angle is not closed.")
    if geometry.arc_count != 3:
        raise RuntimeError("Internal error: generated unequal angle must contain three arcs.")
    return geometry


def to_bentley_curve_vector(geometry, z=0.0):
    """Convert the unequal-angle profile into Bentley native curve primitives."""
    if not geometry.is_closed_and_continuous():
...
def _make_corner(previous, vertex, following, radius):
    ...
    if tangent_length >= _distance(previous, vertex) or tangent_length >= _distance(vertex, following):
        raise ValueError("Unequal-angle fillet radius is too large for the profile.")
...
def _normalize(vector):
    ...
    if length == 0.0:
        raise ValueError("Coincident unequal-angle vertices are invalid.")
