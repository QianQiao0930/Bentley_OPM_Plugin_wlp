...
4. 在“放置方式”中选择：
   ...
   - **沿路径扫掠**：点“选取路径”后在模型中选择一条开放路径线（直线、折线、SmartLine、复杂链、圆弧或样条）。截面自动生成在路径起点、截面平面垂直于路径起点切向，沿路径扫掠为 SmartSolid。此时仅生成**预览**：改型钢型式 / 规格 / 插入基准 / 旋转角都会实时重建预览，点【确定】才保留实体（同时写入公共支吊架统计库），右键 Reset 放弃预览；勾选“扫掠完成后删除路径线”可在确认时删除源路径线。
