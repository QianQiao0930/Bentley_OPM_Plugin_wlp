...
4. 在“放置方式”中选择：
   ...
   - **沿路径扫掠**：在模型中选择一条开放路径线（直线、折线、SmartLine、复杂链、圆弧或样条）。截面会自动生成在路径起点、截面平面垂直于路径起点切向，然后沿路径扫掠为 SmartSolid。默认完成一次后自动退出；勾选“连续扫掠”后可连续选择路径，右键 Reset 或点击“结束工具”退出。还可选择在扫掠成功后自动删除路径线。
