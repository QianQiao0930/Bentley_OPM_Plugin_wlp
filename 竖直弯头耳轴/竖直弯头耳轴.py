...
SUCCESS = 0
ROOT2_OVER_2 = math.sqrt(2.0) / 2.0
...
class VerticalElbowTrunnionBuilder(object):
    ...
    def create_at(self, base_point):
        ...
        # 将 45 度弯曲中点投影到耳轴中心轴，故耳轴在弯头腹部正下方。
        contact_x = -radius + radius * ROOT2_OVER_2
        horizontal_cl_z = plate_t + self.height_h_mm
        start = _point(base_point, -radius - contact_x, 0.0,
                       horizontal_cl_z, self.direction, scale)
        center = _point(base_point, -radius - contact_x, 0.0,
                        horizontal_cl_z + radius, self.direction, scale)
        end = _point(base_point, -contact_x, 0.0,
                     horizontal_cl_z + radius, self.direction, scale)
        contact_cl_z = horizontal_cl_z + radius * (1.0 - ROOT2_OVER_2)
...
class TrunnionDialog(QDialog):
    def __init__(self):
        ...
        self.preview.setStyleSheet(
            "QLabel { background:#eef5ff; color:#174f91; padding:10px; "
            "border:1px solid #cfe1f7; border-radius:6px; }"
        )
        ...
        form.addRow("输出弯头", self.keep_elbow)
        form.addRow("耳轴形式", self.hollow_trunnion)
